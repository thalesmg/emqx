...
-define(UPDATE_SUB_TIMEOUT, 60_000).
-define(CREATE_SUB_TIMEOUT, 60_000).
...
do_update_subscription(Data0) ->
    ...
    maybe
        ...
        %% fixme: take timeout from ttl
        ?keep_state_actions(Data, [?state_timeout(?UPDATE_SUB_TIMEOUT, Handle)])
    else
        {error, Reason} ->
...
do_create_subscription(Data0) ->
    ...
    maybe
        ...
        Data = Data0#{?handle := Handle},
        %% fixme: take timeout from ttl
        ?keep_state_actions(Data, [?state_timeout(?CREATE_SUB_TIMEOUT, Handle)])
    else
        {error, Reason} ->
...
handle_pull_error(Reason, Data0) ->
    #{?source_res_id := SourceResId} = Data0,
    ?tp(warning, "gcp_pubsub_consumer_failed_to_pull", #{
        source_res_id => SourceResId,
        reason => Reason
    }),
    case Reason of
