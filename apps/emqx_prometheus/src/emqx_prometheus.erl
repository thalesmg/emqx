...
emqx_metric_data(MetricNameTypeKeyL, Mode, Acc) ->
    ...
    lists:foldl(
        fun({_Name, _Type, undefined}, AccIn) ->
                AccIn;
...
listener_shutdown_counts(Mode) ->
    ...
    ListenerIds = lists:map(fun({Id, #{bind := Bind}}) -> {Id, Bind} end, Listeners),
    Counts =
        lists:flatmap(
          fun({Id, Bind}) ->
    ...
    CountsByReason =
        lists:foldl(
          fun({Reason, Count}, Acc) ->
            maps:update_with(Reason, fun(C) -> C + Count end, Count, Acc)
          end,
          #{},
          Counts
         ),
    Data =
        lists:map(
          fun({Reason, Count}) -> {with_node_label(Mode, [{reason, Reason}]), Count} end,
          maps:to_list(CountsByReason)
         ),
